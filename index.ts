...
$(() => {
  ...
  const dataInitialization: any = async () => {
    ...
    const favkoins = await chromeStoreApi.getDataFromStore("koinaddafav");
    utility.favKoins = favkoins === undefined ? [] : favkoins;
  ...
  const tabulizeMe = (koins: IKoin[]) => {
    ...
    koins.forEach((ko) => {
      ...
      ko.exchage.forEach((xchange) => {
        tds.push($("<td>").append($("<a>", {
          text: xchange.lastprice + "₹",
          target: "_blank", href: xchange.url,
        })));
      });
